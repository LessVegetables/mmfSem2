...
void popC(charStack *s)
{
    if (s -> root == NULL)      //  used to be: s == NULL
    {
        printf("Stack empty\n");
        return;
    }
...
int isEmptyC(charStack *s)
{
    if (s -> root == NULL) return 1;  // used to be : s == NULL
    else return 0;
}
...
branch *build(char* input)
{
    ...
    for (int i = 0; i < *inputSizePointer; i++)
    {
        ...
        else if (p[input[i]] == 4)              // so p[] == ~
        {
            pushC(cStack, input[i]);
        }
        else if (p[input[i]] > 0 || input[i] == ')')               // 3
        {
            //printf("else if \t 3\n");
            ...
            {
                //printf("while\n");
                //printf("topC(cStack) == %c\t", topC(cStack));
                if (topC(cStack) == '~')
                {
                    //printf("~ detected\n");
                ...
                pushT(nodesStack, t);
                //printf("push(%c)\t", t->data);
                //printf("t: %c\tt1: %c\tt2: %c\n", t->data, t1->data, t2->data);
            }
 
            // Push s[i] to char Stack
            if (input[i] != ')') pushC(cStack, input[i]);
            else popC(cStack);
            //printf("pushC(cStack, input[%d] = %c)\n", i, input[i]);
        }
        else if (input[i] == ')')               // 4
        {
            //printf("else if \t 4\n");
